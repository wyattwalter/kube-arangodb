...
// getLeaderConfig returns agency leader config and client to a leader agency.
func getLeader(ctx context.Context, clients []agency.Agency) (agency.Agency, *agencyConfig, error) {
	...
	for i, cli := range clients {
		go func(iLocal int, cliLocal agency.Agency) {
			...
			if err == nil {
				...
				// There is some error, but maybe it is a follower, and it will be possible to find the leader.
				anyError = err
			}
			mutex.Unlock()
		}(i, cli)
	...
	for i, config := range configs {
		if config.LeaderId == leaderID {
			if config == nil {
				return nil, nil, wrapError(anyError, "empty response from the leader")
			}
